...
public class QuickStartXYTool extends JFrame {
  ...
  public static void main(String[] args) {
    QuickStartXYTool qstart = new QuickStartXYTool();
	  ...
	  public void actionPerformed(ActionEvent ae) {
	    ...
	    if (source == cancel)
	      ...
	      try {
			...
			map.redraw();
			if (QuickStartXYTool.stb.getSelectedLayers() != null)
			  QuickStartXYTool.promoteitem.setEnabled(true);
		  } catch(IOException e){}
	    }
	  }
    };
...
class AddXYtheme extends JDialog {
  ...
  AddXYtheme() throws IOException {
    ...
    //System.out.println("first is "+forest.get(0).get(0));
	setBounds(50,50,520,430);
...
class XYfeatureLayer extends BaseFeatureLayer {
  BaseFields fields;
  String [] attrib = QuickStartXYTool.attrib;
  int fieldCount = 0;
  ...
  private void createFeaturesAndFields(BasePointsArray bpa,Map map,ArrayList<ArrayList> forest,int fieldcount,Vector xCoord,Vector yCoord) {
	...
	createDbfFields();
	System.out.println("hello "+forest.get(0).get(1));
...
class AttrTab extends JDialog {
  JPanel panel1 = new JPanel();
  com.esri.mo2.map.dpy.Layer layer = QuickStartXYTool.layer4;
...
class CreateShapeDialog extends JDialog {
  ...
  ActionListener lis = new ActionListener() {public void actionPerformed(ActionEvent ae) {
	...
	if (o == nameField) {
	  ...
	  //path = ((ShapefileFolder)(QuickStartXYTool.layer4.getLayerSource())).getPath();
...
class CreateAttribDialog extends JDialog {
  JLabel help = new JLabel("<HTML>Please enter column names for your attribute data."+
    ...
    "in the second. AFTER EACH TEXTFIELD ENTRY, HIT ENTER");
  JButton Ok = new JButton("Ok");
  ...
  CreateAttribDialog(){
	//String [] attrib = QuickStartXYTool.attrib;
	setModal(true);
    setBounds (250,550,800,300);
    setTitle("Enter Column Names");
    ActionListener lis = new ActionListener() {public void actionPerformed(ActionEvent ae) {
	  	Object o = ae.getSource();
		if (o == attrib1) {

			QuickStartXYTool.attrib[0] = attrib1.getText().trim();}
	    else if (o == attrib2)
			QuickStartXYTool.attrib[1] = attrib2.getText().trim();
		else if (o == attrib3)
			QuickStartXYTool.attrib[2] = attrib3.getText().trim();
		else if (o == attrib4)
			QuickStartXYTool.attrib[3] = attrib4.getText().trim();
		else if (o == attrib5)
	  	    QuickStartXYTool.attrib[4] = attrib5.getText().trim();
	  	else if (o == attrib6)
	  	    QuickStartXYTool.attrib[5] = attrib6.getText().trim();
	  	else if (o == attrib7)
	  	    QuickStartXYTool.attrib[6] = attrib7.getText().trim();
	  	else if (o == attrib8)
	  	    QuickStartXYTool.attrib[7] = attrib8.getText().trim();
	  	else if (o == attrib9)
	  	    QuickStartXYTool.attrib[8] = attrib9.getText().trim();
	  	else if (o == attrib10)
	  	    QuickStartXYTool.attrib[9] = attrib10.getText().trim();
	  	else if (o == attrib11)
	  	    QuickStartXYTool.attrib[10] = attrib11.getText().trim();
	  	else if (o == attrib12)
	  	    QuickStartXYTool.attrib[11] = attrib12.getText().trim();
	  	else if (o == attrib13)
	  	    QuickStartXYTool.attrib[12] = attrib13.getText().trim();
	  	else if (o == attrib14)
	  	    QuickStartXYTool.attrib[13] = attrib14.getText().trim();
	  	else if (o == attrib15)
	  	    QuickStartXYTool.attrib[14] = attrib15.getText().trim();
	  	else if (o == attrib16)
	  	    QuickStartXYTool.attrib[15] = attrib16.getText().trim();
	  	else if (o == attrib17)
	  	    QuickStartXYTool.attrib[16] = attrib17.getText().trim();
	  	else if (o == attrib18)
	  	    QuickStartXYTool.attrib[17] = attrib18.getText().trim();
	  	else if (o == attrib19)
	  	    QuickStartXYTool.attrib[18] = attrib19.getText().trim();
	    else if (o == attrib20)
	  	    QuickStartXYTool.attrib[19] = attrib20.getText().trim();
	  	else if (o == Ok)
	  	    setVisible(false);
    ...
    panel1.add(help);
	panel1.add(label1);panel1.add(attrib1);
	panel1.add(label2);panel1.add(attrib2);
	panel1.add(label3);panel1.add(attrib3);
	panel1.add(label4);panel1.add(attrib4);
	panel1.add(label5);panel1.add(attrib5);
	panel1.add(label6);panel1.add(attrib6);
	panel1.add(label7);panel1.add(attrib7);
	panel1.add(label8);panel1.add(attrib8);
	panel1.add(label9);panel1.add(attrib9);
	panel1.add(label10);panel1.add(attrib10);
	panel1.add(label11);panel1.add(attrib11);
	panel1.add(label12);panel1.add(attrib12);
	panel1.add(label13);panel1.add(attrib13);
	panel1.add(label14);panel1.add(attrib14);
	panel1.add(label15);panel1.add(attrib15);
	panel1.add(label16);panel1.add(attrib16);
	panel1.add(label17);panel1.add(attrib17);
	panel1.add(label18);panel1.add(attrib18);
	panel1.add(label19);panel1.add(attrib19);
	panel2.add(Ok);
	panel1.add(label20);panel1.add(attrib20);
	getContentPane().add(panel1,BorderLayout.CENTER);
	getContentPane().add(panel2,BorderLayout.SOUTH);
  }
}
class Arrow extends Tool {
  ...
  Arrow() { // undo measure tool residue
    QuickStartXYTool.milesLabel.setText("DIST   0 mi   ");
    QuickStartXYTool.kmLabel.setText("   0 km    ");
    //QuickStartXYTool.map.remove(QuickStartXYTool.acetLayer);
    QuickStartXYTool.acetLayer = null;
    QuickStartXYTool.map.repaint();
  }
}
...
class DistanceTool extends DragTool  {
  ...
  public void mousePressed(MouseEvent me) {
	...
	initPoint = QuickStartXYTool.map.transformPixelToWorld(me.getX(),me.getY());
  }
  public void mouseReleased(MouseEvent me) {
	  // now we create an acetatelayer instance and draw a line on it
	endx = me.getX(); endy = me.getY();
	endPoint = QuickStartXYTool.map.transformPixelToWorld(me.getX(),me.getY());
    ...
    QuickStartXYTool.milesLabel.setText("DIST: " + new Float((float)distance).toString() + " mi  ");
    QuickStartXYTool.kmLabel.setText(new Float((float)(distance*1.6093)).toString() + " km");
    if (QuickStartXYTool.acetLayer != null)
      QuickStartXYTool.map.remove(QuickStartXYTool.acetLayer);
    QuickStartXYTool.acetLayer = new AcetateLayer() {
    ...
    QuickStartXYTool.map.add(QuickStartXYTool.acetLayer);
    QuickStartXYTool.map.redraw();
  }
  public void cancel() {};
}
